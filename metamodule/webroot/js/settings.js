export default class SettingsPage {
        ...
        return `
            <div class="settings-page">
                <div class="settings-header">
                    <h2>${this.app.icon('sliders-h')} Settings & Tools</h2>
                </div>
                    ...
                    <section class="settings-section">
                        ...
                        <div class="action-buttons">
                            <button class="btn btn-secondary" id="view-logs">
                                ${this.app.icon('eye')} View Logs
                            </button>
                            <button class="btn btn-danger" id="clear-logs">
                                ${this.app.icon('trash')} Clear Logs
                            </button>
                        </div>
    ...
    async loadData() {
        try {
            const storageUsage = await this.app.utils.getStorageUsage(CONFIG.IMG_FILE, CONFIG.MNT_DIR);
    ...
    async optimizeStorage(btn) {
        ...
        try {
            this.app.utils.showToast('Optimizing storage... Please wait.', 'info', 5000);
            ...
            this.app.utils.showToast('Storage optimized! Reboot to see changes.', 'success', 4000);

            await this.update();

        } catch (error) {
            this.app.utils.showToast(`Optimization failed: ${error.message}`, 'error');
        } finally {
    ...
    async viewLogs() {
        try {
            ...
            setTimeout(() => {
                ...
                if (refreshBtn && logContent) {
                    refreshBtn.addEventListener('click', async (e) => {
                        ...
                        refreshBtn.innerHTML = this.app.icon('spinner', 'fa-spin') + ' Refreshing';
                        ...
                        try {
                            ...
                            this.app.utils.showToast('Logs refreshed successfully', 'success');
                        } catch (error) {
                            this.app.utils.showToast('Failed to refresh logs', 'error');
                        }
                ...
                if (copyBtn && logContent) {
                    copyBtn.addEventListener('click', async (e) => {
                        e.stopPropagation();
                        await this.app.utils.copyToClipboard(logContent.textContent);
                    });
                }
            }, 100);
            ...
            this.app.utils.showToast(`Failed to read logs: ${error.message}`, 'error');
        }
    }
    ...
    async clearLogs() {
        try {
            ...
            this.app.utils.showToast('Logs cleared successfully', 'success');
        } catch (error) {
            this.app.utils.showToast(`Failed to clear logs: ${error.message}`, 'error');
        }
    }
